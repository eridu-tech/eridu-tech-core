...
- `withAfterCommitFactory(transactionContext)` takes an `ITransactionHooks` implementation, such as a `TransactionContext`, and returns a middleware factory. The optional `settings` argument of type `AfterCommitSettings` is forwarded to `ITransactionHooks.afterCommit()`, so `runWithoutTransaction` decides what happens when no transaction is active: the wrapped function runs immediately (the default) or is discarded.
