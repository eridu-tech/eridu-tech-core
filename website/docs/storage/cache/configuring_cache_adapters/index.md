...
2. Provide a string serializer ([`ISerde`](/docs/foundation/serde)). We recommend using `SuperJsonSerdeAdapter` for this purpose.
...
2. Provide a string serializer ([`ISerde`](/docs/foundation/serde)). We recommend using `SuperJsonSerdeAdapter` for this purpose.
3. Provide a [`TransactionContext`](/docs/foundation/transaction_context/transaction_context_usage) that wraps the `Kysely` instance. The adapter is transaction aware: every cache operation runs through the context's `current` client, so cache reads and writes join the transaction of the current scope.
